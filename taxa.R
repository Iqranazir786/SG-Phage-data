...
setwd("~/Documents/Molecular microbiome analysis/SG/")
list.files()

if(!dir.exists("output"))
  dir.create("output")

metadata<-read_tsv("metadata-SG.txt")
str(metadata)
colnames(metadata)[3] = "Treat"
levels(metadata$Treat)
metadata$Treat.ord = factor(metadata$Treat, c("CM", "CU", "CP", "NC", "PC"))
levels(metadata$Treat.ord)
...
my_column <- "Treat"  #this is the metadata column that we will use in the taxa barplot
...
for(ml in my_level){
  ...
  physeq_meta_filtered$Treat.ord = factor(physeq_meta_filtered$Treat, c("CM", "CP", "CU", "NC", "PC"))
...
diagdds = phyloseq_to_deseq2(physeq_deseq, ~ Treat)
...
alpha = 0.05
my_contrast = c("Treat", "CM", "PC") 
my_contrast = c("Treat", "CP", "PC") 
my_contrast = c("Treat", "CU", "PC") 
my_contrast = c("Treat", "NC", "PC") 
my_contrast = c("Treat", "CM", "NC") 
#my_contrast = c("Treat", "right palm", "left palm") 
