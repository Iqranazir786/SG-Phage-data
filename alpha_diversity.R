...
setwd("~/Molecular microbiome analysis/SG/First necropsy")
...
meta<-read_delim("metadata-SG.txt")
str(meta)
colnames(meta)[4] <- "Treat"
colnames(meta)[3] <- "Necropsy"
...
aov.evenness.Treat = aov(pielou_evenness ~ Treat, data=meta)
aov.faith_pd.Treat = aov(faith_pd ~ Treat, data=meta)
aov.observed_features.Treat = aov(observed_features ~ Treat, data=meta)
aov.shannon.Treat = aov(shannon_entropy ~ Treat, data=meta)
#Call for the summary of that ANOVA, which will include P-values

summary(aov.evenness.Treat)
summary(aov.faith_pd.Treat)
summary(aov.observed_features.Treat)
summary(aov.shannon.Treat)
...
TukeyHSD(aov.evenness.Treat)
TukeyHSD(aov.faith_pd.Treat)
TukeyHSD(aov.observed_features.Treat)
TukeyHSD(aov.shannon.Treat)
...
meta$Treat.ord = factor(meta$Treat, c("NC", "PC", "CU", "CP", "CM"))
levels(meta$Treat.ord)

#Plot
boxplot(pielou_evenness ~ Treat.ord, data=meta, ylab="Peilou evenness")

evenness <- ggplot(meta, aes(Treat.ord, pielou_evenness)) + 
  geom_boxplot() + 
  #facet_grid(~Treat.ord)
  #ylim(c(0.5,1)) +
  theme_q2r() +
...
faithpd <- ggplot(meta, aes(Treat.ord, faith_pd)) + 
  geom_boxplot() + 
  #facet_grid(~Treat.ord)
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("output/faithpd.png", faithpd, height = 3, width = 3)


#boxplot(observed_features ~ Treat.ord, data=meta, ylab="observed_features")
Obs.features <- ggplot(meta, aes(Treat.ord, observed_features)) + 
  geom_boxplot() + (aes(color = Treat.ord))
  #facet_grid(~Treat.ord)
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("output/obs2.png", Obs.features, height = 3, width = 3)

shannons <- ggplot(meta, aes(Treat.ord, shannon_entropy)) + 
  geom_boxplot() + 
  #facet_grid(~Treat.ord)
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))
ggsave("output/shannons.png", shannons, height = 3, width = 3)
...
evenness_summary <- meta %>% # the names of the new data frame and the data frame to be summarised
  group_by(Treat.ord) %>%   # the grouping variable
...
evenness_se <- ggplot(evenness_summary, aes(Treat.ord, mean_evenness, fill = Treat.ord)) + 
  geom_col() + 
...
kruskal.test(faith_pd ~ Treat.ord, data=meta)
...
pairwise.wilcox.test(meta$faith_pd, meta$Treat.ord, p.adjust.method="BH")

# Like evenness, we see that pd also increases with age.

#Plot
boxplot(faith_pd ~ Treat.ord, data=meta, ylab="Faith phylogenetic diversity")

# or with ggplot2

faith_pd <- ggplot(meta, aes(Treat.ord, faith_pd)) + 
  geom_boxplot(aes(color = Treat.ord)) + 
  #ylim(c(0.5,1)) +
  theme_q2r() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5)) +
...
glm.evenness.time = glm(pielou_evenness ~ Treat, data=meta)
...
plot(pielou_evenness ~ Treat, data=meta)
#Add the glm best fit line
plot(pielou_evenness ~ Treat, data=meta) + abline(glm.evenness.time)
...
gaussian.faith.time = glm(faith_pd ~ Treat, data=meta, family="gaussian")
plot(gaussian.faith.time, which=c(1,2))

# Quasipoisson (log) distribution
qp.faith.time = glm(faith_pd ~ Treat, data=meta, family="quasipoisson")
...
#Plot
plot(log(faith_pd) ~ Treat, data=meta, ylab="ln(Faith Phylo. Diversity)")
plot(log(faith_pd) ~ Treat, data=meta, ylab="ln(Faith Phylo. Diversity)") + abline(qp.faith.time)
