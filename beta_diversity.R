...
if(dir.exists("output"))
  print("its there dummy")
  dir.create("output")

#How to load a file into R
metadata2 <- read.delim("metadata-SG.txt", sep = "\t", header = T, quote = "", stringsAsFactors = F)
metadata2[1,]
metadata2[,1]
...
metadata<-read.delim("metadata-SG.txt")
str(metadata)
metadata <- metadata[-1,]
levels(metadata$`Treat`)
colnames(metadata)[3] <- "Necropsy"
colnames(metadata)[4] <- "Treat"
...
body_colors <- c("#e41a1c", "#377eb8", "#4daf4a", "#984ea3", "#ff7f00")
...
#ggplot(data=bc_meta, aes(x=PC1, y=PC2, color=Treat)) +
#  geom_point() + #alpha controls transparency and helps when points are overlapping
#  theme_q2r() +
#  xlab("PC1 (32.27%)") +
#  ylab("PC2 (22.28%)") +
#  scale_color_manual(values=c("Blue", "Black", "Green", "Red", "yellow"), name = "Treat")
...
ggplot(bc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  geom_point(aes(shape= Treat)) + #alpha controls transparency and helps when points are overlapping
  geom_point(data=centroids, size = 3) +
  theme_q2r() +
  #facet_grid(~Treat)
  stat_ellipse(level = 0.95, type = "t") +
  ...
  ylab(paste0("PC2 (", round(100*bc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) 
  #scale_color_manual(values=body_colors, name = my_column)
ggsave(paste0("output/BC-ellipse_", my_column,"-Treat.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  scale_color_manual(values=body_colors, name = "Treat")
ggsave(paste0("output/Wuni-ellipse_", my_column,".pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches

ggplot(Wuni_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  geom_point(aes(shape= Treat), size = 3) + #alpha controls transparency and helps when points are overlapping
  #geom_point(data=centroids, size = 3) +
  theme_q2r() +
  #stat_ellipse(level = 0.95, type = "t") +
  xlab(paste0("PC1 (", round(100*Wuni_PCoA$data$ProportionExplained[1], digits = 2), "%)")) +
  ylab(paste0("PC2 (", round(100*Wuni_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Treat")
ggsave(paste0("output/Wuni-ellipse_", my_column,"Treat.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
...
PERMANOVA_out <- adonis2(bc_dm ~ Treat, data = metadata_sub)

write.table(PERMANOVA_out,"output/Treat_Adonis_overall.csv",sep=",", row.names = TRUE) 
...
PERMANOVA_out <- adonis2(bc_dm ~ body.site*subject, data = metadata_sub)
...
ggplot(jc_meta, aes(x=PC1, y=PC2, color=get(my_column))) +
  ...
  ylab(paste0("PC2 (", round(100*jc_PCoA$data$ProportionExplained[2], digits = 2), "%)")) +
  scale_color_manual(values=body_colors, name = "Treat")
ggsave(paste0("output/jc-ellipse_", my_column,"Treat.pdf"), height=3, width=4.5, device="pdf") # save a PDF 3 inches by 4 inches
